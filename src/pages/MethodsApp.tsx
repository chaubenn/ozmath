...
const MethodsApp: React.FC<MethodsAppProps> = ({ problems, onShowFormulaSheet }) => {
  ...
  const filterProblems = (problem: Problem) => {
    ...
    return matchesType && matchesYear && matchesFormat;
  };

  const handleRandomProblem = () => {
    const filteredProblems = problems.filter(problem => 
      filterProblems(problem) && problem.title.toLowerCase().includes(searchQuery.toLowerCase())
    );
  ...
  const handleStartTest = () => {
    ...
    setCurrentTestIndex(0);
    setReviewMode(false);
    const filteredProblems = problems.filter(filterProblems);
    if (filteredProblems.length > 0) {
  ...
  const handleEndTest = async () => {
    ...
    const selectedProblems = problems.filter(filterProblems);
